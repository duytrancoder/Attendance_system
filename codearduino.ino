  ...
  while(1){
    if(digitalRead(BTN_SELECT)==LOW){ 
      ...
      if(finger.deleteModel(targetID)==FINGERPRINT_OK){
        ...
        display.setCursor(0, 20);
        display.println("⚠ Khong co AS608!");
      } 
      ...
      if (deletedFromSensor) {
        ...
        beepInfo(); // Không có trong sensor nhưng vẫn xóa web
      }
...
// --- VÒNG LẶP CHÍNH ---
void loop() {
  // Check lệnh từ Server mỗi 2 giây (đã giảm từ 5 giây)
