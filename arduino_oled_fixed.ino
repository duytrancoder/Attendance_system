...
// ==========================================================
const char* host = "http://192.168.86.104/chamcongv2/api"; 
...
const unsigned long POLL_INTERVAL = 5000; // 5 giây
...
void enrollNewFinger() {
  ...
  if(finger.createModel()==FINGERPRINT_OK && finger.storeModel(id)==FINGERPRINT_OK){ 
    ...
    display.clearDisplay(); display.setCursor(30, 25); display.println("THANH CONG!"); display.display(); 
...
// ============================================
//  ⚡ HÀM XÓA VÂN TAY (ĐÃ FIX!)
// ============================================
void deleteFingerByID() {
  int targetID=1; bool s=true; delay(300);
  while(s){
    drawIDSelection(targetID);
    if(digitalRead(BTN_UP)==LOW){ beepInfo(); targetID++; if(targetID>127) targetID=127; drawIDSelection(targetID); unsigned long t=millis(); while(digitalRead(BTN_UP)==LOW){ if(millis()-t>1500){targetID+=10; if(targetID>127)targetID=127; beepInfo(); drawIDSelection(targetID); t=millis();} delay(10);} }
...
void deleteAllFingers() {
  ...
  while(1){
    if(digitalRead(BTN_SELECT)==LOW){ 
      ...
      display.setTextSize(1); 
      display.setCursor(0, 0); 
      display.println("Dang xoa tat ca..."); 
      display.display(); 
      ...
      if(finger.emptyDatabase()==FINGERPRINT_OK){
        ...
        // ⚡ BƯỚC 2: Notify server xóa tất cả (QUAN TRỌNG!)
        if (WiFi.status() == WL_CONNECTED) {
          display.setCursor(0, 35);
          display.println("Dang xoa web...");
          display.display();
          ...
          http.begin(url);
          http.setTimeout(10000); // 10 giây (xóa nhiều nên lâu hơn)
          ...
          if (httpCode == 200) {
            ...
            Serial.println("✗ HTTP Error: " + String(httpCode));
            display.setCursor(0, 50);
            display.println("✗ Loi ket noi!");
          }
        } else {
          Serial.println("✗ WiFi not connected!");
          display.setCursor(0, 35);
          display.println("⚠ Mat WiFi!");
          display.println("Chua xoa web!");
        }
...
// --- VÒNG LẶP CHÍNH ---
void loop() {
  // Check lệnh từ Server mỗi 5 giây
